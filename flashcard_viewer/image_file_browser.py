...
IMAGE_TYPES = {".png", ".jpg", ".jpeg", ".webp", ".tiff", ".tif", ".bmp", ".gif", ".avif"}


class ImageFileBrowser(ttk.Toplevel):
    def __init__(self, master, start_path=None):

        super().__init__(master)
        self.title("Select Image File")
    ...
    def build_ui(self):
        ...
        self.preview_canvas.grid(row=1, column=1, sticky="nsew", padx=(5, 10), pady=5)
        ...
        ttk.Button(btn_frame, text="Select File", bootstyle="success", command=self.confirm).pack(side=RIGHT)
    ...
    def load_previews(self, path):
        ...
        try:
            files = sorted(
                (f for f in path.iterdir() if f.is_file() and f.suffix.lower() in IMAGE_TYPES),
        ...
        for file in files:
            ...
            container = ttk.Frame(self.preview_frame, padding=3, cursor="hand2")
            container.grid(row=row, column=col, padx=3, pady=3)

            lbl = ttk.Label(container, image=thumb)
            lbl.image = thumb
            lbl.pack()

            def on_enter(e, c=container):
                if c is not self._selected_container:
                    c.configure(bootstyle="info")

            def on_leave(e, c=container):
                if c is not self._selected_container:
                    c.configure(bootstyle="default")

            def on_click(e, c=container, f=file):
                if self._selected_container is not None:
                    self._selected_container.configure(bootstyle="default")
                self._selected_container = c
                c.configure(bootstyle="primary")
                self.selected_file = f
                self.selected_parent = f.parent
    ...
    def show(self):
        self.wait_window()
        return self.selected_file, self.selected_parent
